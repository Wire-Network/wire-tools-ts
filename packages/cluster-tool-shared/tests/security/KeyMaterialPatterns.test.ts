...
import { findKeyMaterial, KeyMaterialPatterns, KeySpecPatterns, RedactedKeyMarker } from "@wireio/cluster-tool-shared"

/** anvil account #0's PUBLISHED private key — a real 0x + 64-hex secret. */
const AnvilAccountZeroPrivateKey = "0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80"
...
describe("KeyMaterialPatterns", () => {
  ...
  describe("findKeyMaterial detects every secret encoding", () => {
    it("a WIRE canonical PVT_ private key (every curve)", () => {
      const wireCanonical = [
        ...
        PrivateKey.regenerate(KeyType.BLS, new Uint8Array(32).fill(7)).toString()
      ]
      wireCanonical.forEach(privateKey => expect(findKeyMaterial(`{"privateKey":"${privateKey}"}`)).not.toHaveLength(0))
    })
    ...
    it("a 0x-prefixed 32-byte hex secret — EM's chain-native form", () => {
      expect(findKeyMaterial(`{"key":"${AnvilAccountZeroPrivateKey}"}`)).not.toHaveLength(0)
    })
    ...
    it("a 64-element JSON byte array (a solana-keygen keypair file)", () => {
      const bytes = JSON.stringify(Array.from({ length: 64 }, (_unused, index) => index))
    ...
    it("a BIP-39 mnemonic phrase (12 and 24 words)", () => {
      ...
      expect(findKeyMaterial(`${AnvilMnemonic} ${AnvilMnemonic}`)).not.toHaveLength(0)
    })
  })
  ...
  it("names the signature it matched, so a scanner can quote it", () => {
    ...
    expect(matched.map(entry => entry.name)).toContain("0x-prefixed 32-byte hex private key")
  })
  ...
  describe("KeySpecPatterns — the LOG set", () => {
    ...
    it("does NOT flag a spec the collector already redacted", () => {
      expect(findKeyMaterial(spec(RedactedKeyMarker), KeySpecPatterns)).toEqual([])
    })
    ...
    it.each([
      ...
      [
        "prose that looks like a mnemonic",
        "the cluster will now start and then wait until every single node has fully joined"
      ]
