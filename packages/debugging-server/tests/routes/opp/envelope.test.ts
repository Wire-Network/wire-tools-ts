...
//  JSON-RPC 2.0 tests
// -----------------------------------------------------------------------
describe(`JSON-RPC 2.0 via POST ${ApiPaths.OPP.Base}`, () => {
  ...
  function rpcCall(method: string, params: any) {
    const id = nextId++
    return fetch(`${baseUrl}${ApiPaths.OPP.Base}`, {
      method: "POST",
      headers: {
    ...
    }).then(async r => ({ status: r.status, body: await r.json() as any, id }))
  }

  it("returns JSON-RPC 2.0 response with result for valid call", async () => {
    const { status, body, id } = await rpcCall(ApiPaths.OPP.Envelope, {
  ...
  it("returns INVALID_REQUEST when jsonrpc field is missing", async () => {
    const resp = await fetch(`${baseUrl}${ApiPaths.OPP.Base}`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ method: ApiPaths.OPP.Envelope, params: {} })
    })
  ...
  it("echoes the request id in the response", async () => {
    const { body, id } = await rpcCall(ApiPaths.OPP.Envelope, {
  ...
  it("deduplicates and appends batch_op_name via JSON-RPC", async () => {
    ...
    const r1 = await rpcCall(ApiPaths.OPP.Envelope, {
      ...params,
      batchOpName: "batchop.a"
    })
    const r2 = await rpcCall(ApiPaths.OPP.Envelope, {
      ...params,
...
// -----------------------------------------------------------------------
describe(`Plain JSON via POST ${ApiPaths.OPP.Envelope}`, () => {
  ...
  it("accepts plain JSON body and returns plain result", async () => {
    const resp = await fetch(`${baseUrl}${ApiPaths.OPP.Envelope}`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
...
//  List envelopes
// -----------------------------------------------------------------------
describe(`JSON-RPC ${ApiPaths.OPP.EnvelopeList}`, () => {
  ...
  beforeAll(async () => {
    ...
    for (const [epoch, ep, name] of [
      ...
      await rpcCall(ApiPaths.OPP.Envelope, {
        batchOpName: name,
        endpointsType: ep,
  ...
  it("returns all envelopes when no filters applied", async () => {
    const { body } = await rpcCall(ApiPaths.OPP.EnvelopeList, {})
...
//  Get envelope
// -----------------------------------------------------------------------
describe(`JSON-RPC ${ApiPaths.OPP.EnvelopeGet}`, () => {
  ...
  beforeAll(async () => {
    ...
    // Seed one envelope
    const { body } = await rpcCall(ApiPaths.OPP.Envelope, {
      batchOpName: "batchop.a",
      endpointsType: DebugOutpostEndpointsType.OUTPOST_SOLANA_DEPOT,
  ...
  it("returns the full envelope for a valid key", async () => {
    const { body } = await rpcCall(ApiPaths.OPP.EnvelopeGet, { key: storedKey })
  ...
  it("returns an error for a nonexistent key", async () => {
    const { body } = await rpcCall(ApiPaths.OPP.EnvelopeGet, { key: "00000000-FAKE-0000" })
    expect(body.error).toBeDefined()
