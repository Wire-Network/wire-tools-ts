...
export namespace LogRoutes {
   ...
   */
  export function register(registry: JsonRPC.HandlerRegistry, clusterPath: string): JsonRPC.HandlerRegistry {
    JsonRPC.addRoute(registry, ApiPaths.Logs.Methods.GetStat, async (params: LogStatRequest): Promise<LogStat> => {
    ...
    JsonRPC.addRoute(registry, ApiPaths.Logs.Methods.Read, async (params: LogReadRequest): Promise<LogReadResponse> => {
      assertUnderClusterPath(params.path, clusterPath)
      const idx = await buildLineIndex(params.path),
...
function assertUnderClusterPath(path: string, clusterPath: string): void {
  if (!isPathUnder(path, clusterPath)) {
    throw new Error(`Path traversal rejected: ${path} is not under ${clusterPath}`)
  }
}
