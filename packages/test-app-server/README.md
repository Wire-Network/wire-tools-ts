...
- A complete `wire-sysio` build including all programs (e.g. `/data/shared/code/wire/wire-sysio/build/debug`)
- Node.js and pnpm
...
The app needs a K1 private key for signing transactions. Set `AUTHEX_TEST_PRIVATE_KEY` in a `.env` file in **this directory** (`wire-tools-ts/packages/test-app-server/.env`) or at the **wire-tools-ts repo root** (`wire-tools-ts/.env`):
...
|------|----------|-------------|
| `-B <build-dir>` | Yes | **Absolute** path to the wire-sysio build directory (or relative to `$PWD`). Must contain `programs/clio/clio`. |
...
```bash
./dev-setup.sh -B /data/shared/code/wire/wire-sysio/build/debug-clang-clion \
  --private-key PVT_K1_<private-key> \
  --link-username john.doe
```

The script will:
1. Create the `dev.owner1` and `<link-username>` accounts on chain (if they don't already exist)
2. Register `dev.owner1` as a tier-1 node owner in `sysio.roa`
3. Add a resource policy for `<link-username>` under `dev.owner1`

## Running the Dev Server

```bash
# From the wire-tools-ts monorepo root:
pnpm install
...
| `pnpm clean` | Remove `lib/` and `dist/` build artifacts |
