...
usage() {
   echo "Usage: $0 -B <build-dir> --private-key <private-key> [--url <nodeop-url>] [--link-username <link-username>]"
   echo ""
   echo "  -B <build-dir>              Required. Path to build folder (absolute or relative to repo root)."
   echo "  --private-key <private-key>  Required. K1 private key to derive the public key from."
...
# Resolve build dir to absolute path (relative to $PWD)
if [[ "$BUILD_DIR" != /* ]]; then
   BUILD_DIR="$(cd "$PWD/$BUILD_DIR" 2>/dev/null && pwd)" \
     || { echo "Error: -B path '$BUILD_DIR' does not exist (resolved relative to \$PWD)."; exit 1; }
fi
...
if [[ -n "$PRIVATE_KEY" ]]; then
  ...
  die "--private-key <PRIVATE_KEY> is required"
fi
