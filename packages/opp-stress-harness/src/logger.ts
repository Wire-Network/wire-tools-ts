 ...
 * - every other logger (`getLogger(...)` diagnostics) → a formatted line on
 *   **stderr**, keeping stdout a pure data channel.
 *
 * This matters concretely for this CLI: without the routing appender the
 * `@wireio/shared` default `ConsoleAppender` sends `log.info` through
 * `console.info` — i.e. onto STDOUT — so `wire-opp-stress saturation --json |
 * jq` interleaves diagnostics with the JSON payload it is meant to pipe.
 *
 * Installed once (module load) via `setAppenders`, preempting that lazy
 * `ConsoleAppender` (which would ALSO print the stdout category and double up).
 */
 ...
 * data channel (summaries, `--json` payloads). Per-file diagnostics still go
 * through `getLogger(...)`; this is the separate data channel.
 *
