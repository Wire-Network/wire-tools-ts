...
Core library and CLI for creating, running, and tearing down multi-chain WIRE test clusters. Ships the `wire-cluster-tool` binary, process managers for every cluster component, and typed clients for WIRE / Ethereum / Solana.

- **Binary:** `wire-cluster-tool`
...
A "cluster" is an on-disk directory plus the long-running processes that operate on it. `wire-cluster-tool` owns the full lifecycle:
...
After this the `wire-cluster-tool` bin is linked into `node_modules/.bin/`. Invoke via `pnpm exec wire-cluster-tool …` or the workspace-wide `pnpm wire-cluster-tool …`.
...
```
wire-cluster-tool [global-options] <command> [command-options]
```
...
```bash
wire-cluster-tool --cluster-path /data/opt/wire/chains/dev-001 run
```
...
```bash
wire-cluster-tool --cluster-path /data/opt/wire/chains/dev-full run
```
...
```bash
wire-cluster-tool --cluster-path /data/opt/wire/chains/dev-full destroy
```
...
Once `wire-cluster-tool … run` is live in one terminal, use the sibling **`wire-debugging-client-tool-tui`** TUI in a second terminal to observe it. The TUI reads the same on-disk layout — `cluster-config.json`, `cluster-state.json`, per-process pid files, per-process logs, and OPP envelopes under `data/opp-debugging/` — so there is zero extra setup.
...
See the `flow-operator-collateral-deposit` / `flow-swap-with-underwriting` / `flow-batch-operator-termination` / `flow-swap-variance-revert` packages for end-to-end test examples that drive full scenarios against a harness-built cluster.
...
pnpm --filter @wireio/cluster-tool compile:watch

# Unit tests
pnpm --filter @wireio/cluster-tool test

# Prettier
pnpm --filter @wireio/cluster-tool format
```
