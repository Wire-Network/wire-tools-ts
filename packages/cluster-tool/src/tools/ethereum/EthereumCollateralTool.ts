...
import { ethereumCompressedPubkey, ethereumSigner } from "../../utils/keyPairUtils.js"
import { Report } from "../../report/Report.js"
import { retry } from "../../utils/asyncUtils.js"
import { contractView, ethereumRevertReason, resolveLatestNonce } from "../../utils/ethereumUtils.js"
...
export interface OperatorRegistryContract extends ethers.BaseContract {
  ...
  depositNonNative: (...args: DepositNonNativeArgs) => Promise<ethers.ContractTransactionResponse>
  withdraw: (
    compressedPubkey: string | Uint8Array,
    tokenCode: bigint,
    amount: bigint,
...
export namespace EthereumCollateralTool {
  ...
  export async function runErc20Approval<C extends ClusterBuildContext>(
    ...
    signal.throwIfAborted()
    const operator = ctx.keyStore.assertOperator(input.operatorLabel)
    const tokenAddress = mockErc20Address(ClusterConfigProvider.ethereumDeploymentsPath(ctx.config), input.tokenName)
    const spender = loadOutpostAddresses(ClusterConfigProvider.ethereumDeploymentsPath(ctx.config)).OperatorRegistry
    Assert.ok(
    ...
    const receipt = await response.wait(1)
    Assert.ok(receipt?.status === 1, `EthereumCollateralTool.approve: reverted (status=${receipt?.status ?? "null"})`)
  }
  ...
  export async function runNonNativeDeposit<C extends ClusterBuildContext>(
    ...
    await retry(
      ...
      throw new NestedError("EthereumCollateralTool.runNonNativeDeposit: depositNonNative would revert", {
        cause: error,
        context: {
  ...
  const Erc20ApproveAbi: ethers.InterfaceAbi = ["function approve(address spender, uint256 amount) returns (bool)"]
  ...
  export function loadOperatorRegistry<C extends ClusterBuildContext>(
    ...
    const address = loadOutpostAddresses(ClusterConfigProvider.ethereumDeploymentsPath(ctx.config)).OperatorRegistry
    Assert.ok(
  ...
  export function loadOutpostAddresses(deploymentsPath: string): Record<string, string> {
    ...
    Assert.ok(Fs.existsSync(addressesPath), `EthereumCollateralTool: outpost addresses not found at ${addressesPath}`)
   ...
   */
  export function mockErc20Address(deploymentsPath: string, tokenName: string): string {
    const addresses = loadOutpostAddresses(deploymentsPath),
      address = match(tokenName)
        ...
        .with("LIQETH", () => addresses.LiqEth ?? addresses.LiqETH ?? addresses.LiqEthToken)
        .otherwise(() => null)
    Assert.ok(
      address != null && /^0x[0-9a-fA-F]{40}$/.test(address),
  ...
  export function loadOutpostAbi(ethereumPath: string, contractName: string): ethers.InterfaceAbi {
    ...
    Assert.ok(Fs.existsSync(artifactPath), `EthereumCollateralTool: ABI artifact not found at ${artifactPath}`)
