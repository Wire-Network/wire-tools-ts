...
import { resolveLatestNonce, type EthereumPayableOverrides } from "../../utils/ethereumUtils.js"
 ...
 */
export interface ReserveManagerRequestSwapContract extends ethers.BaseContract {
  requestSwap: (
...
export async function requestEthereumSwap(
  ...
  Assert.ok(request.sourceAmountWei > 0n, "EthereumSwapTool: sourceAmountWei must be > 0")
  Assert.ok(request.targetRecipient.byteLength > 0, "EthereumSwapTool: targetRecipient must be non-empty")
  Assert.ok(request.targetAmount > 0n, "EthereumSwapTool: targetAmount must be > 0")
  Assert.ok(
...
export interface ReserveManagerErc20SwapContract extends ethers.BaseContract {
...
export async function requestEthereumSwapErc20WithPermit(
  ...
  Assert.ok(args.sourceAmount > 0n, "EthereumSwapTool: sourceAmount must be > 0")
  Assert.ok(args.targetRecipient.byteLength > 0, "EthereumSwapTool: targetRecipient must be non-empty")
  Assert.ok(args.targetAmount > 0n, "EthereumSwapTool: targetAmount must be > 0")
  Assert.ok(
 ...
 * one-byte storage enum: PENDING=0, ACTIVE=1, CANCELLED=2). Deliberately
 * DISTINCT from the depot's proto `ReserveStatus` (UNKNOWN=0, PENDING=1,
 * ACTIVE=2, CANCELLED=3) — the translation happens at the outpost dispatch
 * boundary, so substituting the proto enum would shift every comparison by
 * one. Consumed by the reserve flows when reading the outpost's local
 * `getReserve(...)` record.
 */
export enum EthereumLocalReserveStatus {
  PENDING = 0,
  ACTIVE = 1,
  CANCELLED = 2
}
...
export async function requestEthereumReserveCreateErc20WithPermit(
  ...
  Assert.ok(args.externalTokenAmount > 0n, "EthereumSwapTool: externalTokenAmount must be > 0")
  Assert.ok(args.reserveCode !== 0n, "EthereumSwapTool: reserveCode must be non-zero")
