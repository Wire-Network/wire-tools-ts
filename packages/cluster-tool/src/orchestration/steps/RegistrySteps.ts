...
const { SysioContractName, SysioChainsChainkind, SysioTokensTokenkind, SysioTokensChainkind } = SysioContracts
...
export namespace RegistrySteps {
   ...
   */
  export const MockReserveRegistrations: SysioContracts.SysioReservRegreserveAction[] = MockReservePairs.map(
    ([chainCodename, tokenCodename, label]) => toReserveRegistration(chainCodename, tokenCodename, label)
  )
  ...
  export function planSeedRegistry<C extends ClusterBuildContext = ClusterBuildContext>(
    ...
    return ClusterBuildStep.create<C, null>(actor, name, description, options, null, runSeedRegistry)
  }
   ...
   */
  export function planMockReserves<C extends ClusterBuildContext = ClusterBuildContext>(
    parent: ClusterBuildParent<C>,
    name: string,
    description: string,
    options: ClusterBuildStepOptions
  ): ClusterBuildPhase<C> {
    const steps: ClusterBuildStep.Any<C>[] = MockReservePairs.map(([chainCodename, tokenCodename], index) =>
  ...
  export async function runSeedRegistry<C extends ClusterBuildContext>(
    ...
    const chains = ctx.wire.getSysioContract(SysioContractName.chains),
      ...
      ethereumAddresses = readJson(
        Path.join(ClusterConfigProvider.ethereumDeploymentsPath(ctx.config), "outpost-addrs.json")
      ),
      liqEthAddresses = readJson(
        Path.join(ClusterConfigProvider.ethereumDeploymentsPath(ctx.config), "liqeth-addrs.json")
      ),
    ...
    const chainRegistrations: SysioContracts.SysioChainsRegchainAction[] = [
      ...
      },
      {
        ...
        external_chain_id: ctx.config.externalOutposts?.ethereum.chainId ?? AnvilProcess.DefaultChainId,
        name: "Ethereum (anvil)",
  ...
  function toReserveRegistration(
    ...
    return {
      ...
      initial_chain_amount: stable ? ReserveSeedAmount / StableChainSeedDivisor : ReserveSeedAmount,
      initial_wire_amount: ReserveSeedAmount,
      source_token_precision: stable ? StableTokenPrecision : DefaultTokenPrecision,
  ...
  function readSolanaMints(file: string): Record<string, string> {
    ...
    const rows = JSON.parse(Fs.readFileSync(file, "utf-8")) as SolanaFundingTool.SolMockMint[]
  ...
  function nullableMintHex(mintBase58: string | null): string {
    return mintBase58 != null ? Buffer.from(new SolanaPublicKey(mintBase58).toBytes()).toString("hex") : ""
  }
}
