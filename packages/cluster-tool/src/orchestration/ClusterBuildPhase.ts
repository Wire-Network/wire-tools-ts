 ...
 * parallel when `options.parallelize`. Fail-fast mode short-circuits the
 * remainder and cancels in-flight siblings on the first failure; collect mode
 * leaves independent steps running. Steps never throw out of here: each becomes
 * a {@link Report.StepResult}.
 */
          ...
          const results = await Bluebird.map(
            ...
            step => this.runStep(step, controller),
            {
              concurrency:
                ...
                ClusterBuildPhase.UnboundedConcurrency
            }
          )
