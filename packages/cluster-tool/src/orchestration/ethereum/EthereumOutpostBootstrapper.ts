...
export class EthereumOutpostBootstrapper {
  ...
  constructor(options: EthereumOutpostBootstrapperOptions) {
    ...
    Assert.ok(options.deploymentsPath, "EthereumOutpostBootstrapper: deploymentsPath is required")
  ...
  private async deployContracts(ethereumPath: string, rpcUrl: string): Promise<void> {
    ...
    const deployerPrivateKey = this.accounts[EthereumOutpostBootstrapper.DeployerAccountIndex].privateKey
    this.markAccountUsed(EthereumOutpostBootstrapper.DeployerAccountIndex, "Contract deployer (LiqEth + Outpost)")
    ...
    const { stdout, stderr } = await withFileLock(
      ...
      () =>
        execFileAsync("npx", ["hardhat", "run", "src/scripts/deployLocal.ts", "--network", "localhost"], {
          cwd: ethereumPath,
  ...
  private async seedReserveManager(rpcUrl: string): Promise<void> {
    const outpostAddressesFile = Path.join(this.config.deploymentsPath, "outpost-addrs.json")
    ...
    const { ReserveManager: reserveManagerAddress, MockUsdc: mockUsdcAddress, MockUsdt: mockUsdtAddress } = addresses
...
export namespace EthereumOutpostBootstrapper {
   ...
   */
  export const HardhatDeployLockPath = Path.join(Os.tmpdir(), "wire-ethereum-hardhat-deploy.lock")
