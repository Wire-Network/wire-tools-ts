...
describe("WireSysioContractTool helpers", () => {
  it("sysioActiveAuthority is sysio@active only, no keys", () => {
    ...
    expect(authority.accounts).toEqual([{ permission: { actor: "sysio", permission: "active" }, weight: 1 }])
  })

  it("sysioActiveCodeAuthority puts sysio@active first, then code accounts sorted by name", () => {
    const authority = WireSysioContractTool.sysioActiveCodeAuthority(["sysio.opreg", "sysio.msgch"])
    ...
    expect(codeAccounts.map(entry => entry.permission.actor)).toEqual(["sysio.msgch", "sysio.opreg"])
    codeAccounts.forEach(entry => expect(entry.permission.permission).toBe("sysio.code"))
  })
