import { maskSecretArgs, RedactedMarker } from "@wireio/cluster-tool/utils/secretUtils"
...
describe("maskSecretArgs", () => {
  it("masks the value FOLLOWING --private-key, keeping the rest verbatim", () => {
    expect(
      maskSecretArgs(["/build/bin/clio", "wallet", "import", "-n", "default", "--private-key", PrivateKey])
    ).toEqual(["/build/bin/clio", "wallet", "import", "-n", "default", "--private-key", RedactedMarker])
  })

  it("masks the value following --password", () => {
    const masked = maskSecretArgs(["/build/bin/clio", "wallet", "unlock", "--password", "PW5KQwrpb..."])
    expect(masked.at(-1)).toBe(RedactedMarker)
    expect(masked).toContain("--password")
  })

  it("masks an INLINE KEY: spec while preserving the public key before it", () => {
    const [, spec] = maskSecretArgs(["/build/bin/nodeop", `--signature-provider=${PublicKey}=KEY:${PrivateKey}`])
