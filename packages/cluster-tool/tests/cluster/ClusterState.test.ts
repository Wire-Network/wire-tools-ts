...
describe("ClusterState", () => {
  ...
  describe("save / load round-trip (cluster-state.json)", () => {
    ...
    it("throws on a version mismatch", () => {
      const ctx = seededContext()
      ClusterState.save(ctx.config, ClusterState.capture(ctx))
      const file = ClusterState.stateFilePath(ctx.config)
      const raw = JSON.parse(Fs.readFileSync(file, "utf8"))
      Fs.writeFileSync(file, JSON.stringify({ ...raw, version: 99 }))
      expect(() => ClusterState.load(ctx.config)).toThrow(/version/)
    })
  })
  ...
  describe("captureKeys / save / load round-trip (cluster-keys.json)", () => {
    it("round-trips node key sets and every operator's full key set", () => {
      ...
      const operator = loaded.operators.find(entry => entry.account === BatchOperatorAccount)
    ...
    it("throws on a version mismatch", () => {
      const ctx = seededContext()
      ClusterState.saveKeys(ctx.config, ClusterState.captureKeys(ctx))
      const file = ClusterState.keysFilePath(ctx.config)
      const raw = JSON.parse(Fs.readFileSync(file, "utf8"))
      Fs.writeFileSync(file, JSON.stringify({ ...raw, version: 99 }))
      expect(() => ClusterState.loadKeys(ctx.config)).toThrow(/version/)
    })
  })
