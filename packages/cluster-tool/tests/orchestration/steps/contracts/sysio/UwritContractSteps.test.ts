...
describe("Steps.contracts.sysio.uwrit", () => {
  it("setconfig carries the uwrit::setconfig data", () => {
    ...
    // escrow floor `min_fromwire_amount` + caller-fault drain-revert fee
    // `fromwire_revert_fee_bps`) and the SEC-129 / WSA-223 UWREQ lifecycle
    // knobs (`uwreq_pending_timeout_epochs` + `uwreq_retention_epochs`). The
    // action type requires all six fields — a payload missing any pair fails
    ...
    const data: SysioContracts.SysioUwritSetconfigAction = {
      ...
      uwreq_pending_timeout_epochs: 10,
      uwreq_retention_epochs: 10
    }
    ...
    expect(step.input.data.uwreq_pending_timeout_epochs).toBe(10)
    expect(step.input.data.uwreq_retention_epochs).toBe(10)
