...
describe("pollUntil.timeoutScale", () => {
  ...
  it("stretches the deadline (timeout message carries the scaled budget)", async () => {
    ...
    await expect(pollUntil("never", async () => false, 20, 5)).rejects.toThrow("Timed out waiting for: never (60ms)")
  })
...
describe("pollUntil", () => {
  ...
  it("throws a labelled timeout when the deadline elapses", async () => {
    await expect(pollUntil("never", async () => false, 30, 5)).rejects.toThrow(/Timed out waiting for: never \(30ms\)/)
  })
})

describe("verifyStep", () => {
  it("builds an input-less step attributed to the actor, with a runner", () => {
    const step = verifyStep(Report.Actor.Sysio, "verify-thing", "verify a thing holds", async () => undefined)
  ...
  it("runner delegates to the verification fn (a throw surfaces)", async () => {
    const step = verifyStep(Report.Actor.User, "failing-check", "a check that fails", async () => {
      throw new Error("boom")
    })
    await expect(step.runner(null as never, null, new AbortController().signal)).rejects.toThrow("boom")
  })
})
...
describe("pollStep.lift", () => {
  it("returns a fn that polls the predicate to resolution", async () => {
    let calls = 0
    const fn = pollStep.lift<ClusterBuildContext>("counter reaches 2", async () => ++calls >= 2, 1_000, 2)
