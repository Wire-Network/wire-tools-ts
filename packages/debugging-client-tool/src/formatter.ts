import type { EnvelopeListEntry, GetEnvelopeResponse } from "@wire-e2e-tests/debugging-shared"
import { DebugOutpostEndpointsType, endpointsTypeToKey } from "@wire-e2e-tests/debugging-shared"
import { Envelope, Endpoints } from "@wireio/opp-typescript-models"
...
export function formatInspect(resp: GetEnvelopeResponse, format: OutputFormat): string {
   ...
   if (resp.envelopeData && resp.envelopeData.length > 0) {
      try {
         ...
         lines.push(`  Epoch Timestamp: ${formatTimestamp(Number(envelope.epochTimestamp))}`)
         lines.push(`  Envelope Hash:   ${bufToHex(envelope.envelopeHash)}`)
         lines.push(`  Previous Hash:   ${bufToHex(envelope.previousEnvelopeHash)}`)
         ...
         for (let i = 0; i < envelope.messages.length; i++) {
            ...
            if (msg.header) {
               ...
               lines.push(`    Prev Msg ID:   ${bufToHex(msg.header.previousMessageId)}`)
               lines.push(`    Timestamp:     ${formatTimestamp(Number(msg.header.timestamp))}`)
            }
            if (payload) {
               ...
               for (let a = 0; a < payload.attestations.length; a++) {
                  ...
                  lines.push(`      [${a}] type=${att.type} data_size=${att.dataSize}`)
               }
            }
         }
      } catch (err: any) {
...
function bufToHex(buf: Uint8Array | undefined): string {
   ...
   return Buffer.from(buf).toString("hex").substring(0, 32) +
      (buf.length > 16 ? "..." : "")
}
