...
function producerName(index: number): string {
...
export class ClusterManager {
  ...
  async create(config: ClusterConfig): Promise<void> {
    ...
    const dataPath = Path.join(chainDir, "data")
    const walletDir = Path.join(chainDir, "wallet")
    const launchTime = new Date().toISOString().replace("Z", "").slice(0, 23)

    this.pm.setClusterDir(chainDir)
    ...
    Fs.mkdirSync(Path.join(dataPath, "node_bios"), { recursive: true })
    for (let i = 0; i < cfg.nodeCount; i++) {
      Fs.mkdirSync(Path.join(dataPath, `node_${String(i).padStart(2, "0")}`), { recursive: true })
    }
    for (let i = 0; i < cfg.batchOperatorCount; i++) {
      Fs.mkdirSync(Path.join(dataPath, `node_batchop_${String(i).padStart(2, "0")}`), { recursive: true })
    }
    for (let i = 0; i < cfg.underwriterCount; i++) {
      Fs.mkdirSync(Path.join(dataPath, `node_uwrit_${String(i).padStart(2, "0")}`), { recursive: true })
    }
    Fs.mkdirSync(Path.join(dataPath, "anvil", "state"), { recursive: true })
    Fs.mkdirSync(Path.join(dataPath, "solana_validator"), { recursive: true })
    ...
    await waitForEndpoint(`${kiodUrl}/v1/wallet/list_wallets`, { label: "kiod", timeoutMs: 10_000 })

    const clioBin = Path.join(buildDir, "bin", "clio")
    const clioWallet = new Clio({ binary: clioBin, url: "http://127.0.0.1:8788", walletUrl: kiodUrl })
    ...
    log.info(`Generated and imported keys for ${cfg.nodeCount} producer(s), ${cfg.batchOperatorCount} batch op(s), ${cfg.underwriterCount} underwriter(s)`)
    ...
    for (let i = 0; i < cfg.producerCount; i++) {
      allProducerNames.push(producerName(i))
    }
    // Assign producers round-robin across nodes (non-consecutive, matching Python)
    const nodeProducers: string[][] = Array.from({ length: cfg.nodeCount }, () => [])
    ...
    for (let i = 0; i < cfg.nodeCount; i++) {
      allPeerAddresses.push(`localhost:${P2P_PORT_BASE + i}`)
    }
    ...
    const writeNodeFiles = (
      ...
      const defaultIniPath = Path.join(buildDir, "etc", "sysio", "node_bios", "config.ini")
      let configIni = ""
    ...
    // ── 5a. Bios node ──
    const biosDir = Path.join(dataPath, "node_bios")
    ...
    for (let i = 0; i < cfg.nodeCount; i++) {
      const nodeDir = Path.join(dataPath, `node_${String(i).padStart(2, "0")}`)
    ...
    for (let i = 0; i < cfg.batchOperatorCount; i++) {
      ...
      const nodeDir = Path.join(dataPath, `node_batchop_${String(i).padStart(2, "0")}`)
      const nodeGenesisPath = Path.join(nodeDir, "genesis.json")
      ...
      const cmd = buildStartCmd({
        ...
        genesisJson: nodeGenesisPath,
        genesisTimestamp: launchTime,
        p2pMaxNodesPerHost: cfg.nodeCount + cfg.batchOperatorCount + cfg.underwriterCount + 1,
    ...
    for (let i = 0; i < cfg.underwriterCount; i++) {
      ...
      const nodeDir = Path.join(dataPath, `node_uwrit_${String(i).padStart(2, "0")}`)
      const nodeGenesisPath = Path.join(nodeDir, "genesis.json")
      const httpPort = HTTP_PORT_BASE + portOffset
      const p2pPort = P2P_PORT_BASE + portOffset
      const peers = allPeerAddresses.filter(a => a !== `localhost:${p2pPort}`)
    ...
    log.info(`Generated files for bios + ${cfg.nodeCount} producer(s) + ${cfg.batchOperatorCount} batch op(s) + ${cfg.underwriterCount} underwriter(s)`)

    // ── 6. Start bios node ──
    await this.launchFromCmd("node-bios", biosCmd, biosDir)
    const biosHttpUrl = `http://127.0.0.1:${BIOS_HTTP_PORT}`
    await waitForEndpoint(`${biosHttpUrl}/v1/chain/get_info`, { label: "bios-node", timeoutMs: 30_000 })
    ...
    for (const ns of nodeStates) {
      ...
      await sleep(2000) // stagger starts (matches Python delay=2)
    }
    ...
    await bootstrapChain(clio, cfg, biosHttpUrl, nodeStates, batchOpStates, underwriterStates)

    // ── 10. Kill bios node (not needed after bootstrap) ──
    log.info("Killing bios node (not needed after bootstrap)...")
    const biosHandle = this.pm.get("node-bios")
    ...
    const clusterState: ClusterState = {
      ...
      anvilStateDir: Path.join(dataPath, "anvil", "state"),
      solanaLedgerDir: Path.join(dataPath, "solana_validator"),
    }
  ...
  async start(): Promise<void> {
    if (!this.state) {
      throw new Error("No cluster state loaded. Call create() or loadState() first.")
    }

    this.pm.setClusterDir(this.chainDir)
    log.info(`Starting ${this.state.nodes.length} nodes...`)

    for (const ns of this.state.nodes) {
      const relaunchCmd = buildRelaunchCmd(ns.cmd)
      const launchTime = new Date().toISOString().replace(/[:.]/g, "_").slice(0, 19)
      ...
      log.info(`  Starting node ${ns.nodeId} (port ${ns.port}): ${ns.producerName ?? "non-producer"}`)
      await this.pm.spawn({
        label: `node-${ns.nodeId}`,
      ...
      try { Fs.unlinkSync(errLink) } catch {}
      try { Fs.symlinkSync(Path.basename(errFile), errLink) } catch {}
    }
    ...
    for (const ns of (this.state.batchOperatorNodes ?? [])) {
      const relaunchCmd = buildRelaunchCmd(ns.cmd)
      log.info(`  Starting batch op ${ns.nodeId} (port ${ns.port}): ${ns.operatorAccount}`)
      await this.pm.spawn({
        label: `node-${ns.nodeId}`,
        command: relaunchCmd[0],
        args: relaunchCmd.slice(1),
    ...
    for (const ns of (this.state.underwriterNodes ?? [])) {
      const relaunchCmd = buildRelaunchCmd(ns.cmd)
      log.info(`  Starting underwriter ${ns.nodeId} (port ${ns.port}): ${ns.operatorAccount}`)
      await this.pm.spawn({
        label: `node-${ns.nodeId}`,
        command: relaunchCmd[0],
        args: relaunchCmd.slice(1),
    ...
    if (this.state.anvilStateDir) {
      const anvilManager = new AnvilManager(this.pm, {
    ...
    if (this.state.solanaLedgerDir) {
      const solManager = new SolanaValidatorManager(this.pm, {
  ...
  private async launchFromCmd(label: string, cmd: string[], cwd: string): Promise<void> {
    await this.pm.spawn({
      label,
      command: cmd[0],
      args: cmd.slice(1),
...
async function bootstrapChain(
  ...
  await retry(
    () => clio.setContractAndWait("sysio", biosContractDir, "sysio.bios.wasm", "sysio.bios.abi"),
  ...
  if (!featuresResp.ok) throw new Error(`Failed to get protocol features: ${featuresResp.statusText}`)
  ...
  for (const feature of featureList) {
    ...
    if (!digest) continue
    const codename = feature.specification?.find(s => s.name === "builtin_feature_codename")?.value
    ...
    try {
      ...
      if (!msg.includes("already activated") && !msg.includes("already been activated")) {
        log.warn("[Phase 3] Feature activation issue: {} - {}", codename ?? digest, msg)
      }
    }
  }
  await sleep(1000)
  ...
  try {
    const finalizerNodes: Array<{ description: string; weight: number; public_key: string; pop: string }> = []
    for (const ns of nodeStates) {
      ...
      try {
        ...
        if (resp.ok) {
          const info = await resp.json() as { finalizer_keys?: Array<{ public_key: string; proof_of_possession: string }> }
      ...
      } catch { /* node may not support this endpoint */ }
    }
    if (finalizerNodes.length > 0) {
      const threshold = Math.floor((finalizerNodes.length * 2) / 3) + 1
      await clio.pushAction("sysio", "setfinalizer", JSON.stringify({
        finalizer_policy: { threshold, finalizers: finalizerNodes },
      }), "sysio@active")
      log.info(`[Phase 4] Set ${finalizerNodes.length} finalizers (threshold=${threshold})`)
    } else {
      log.info("[Phase 4] No finalizer keys available, skipping instant finality")
    }
  } catch (err: unknown) {
    const msg = err instanceof Error ? err.message : String(err)
  ...
  for (const acctName of SYSTEM_ACCOUNTS) {
    try {
      ...
      const msg = err instanceof Error ? err.message : String(err)
      if (!msg.includes("already exists")) throw new Error(`Failed to create ${acctName}: ${msg}`)
    }
  }
  ...
  await retry(
    () => clio.setContractAndWait("sysio", resolveContractDir("sysio.system"), "sysio.system.wasm", "sysio.system.abi"),
  ...
  function extractNodeK1PubKey(ns: NodeState): string {
    ...
    return DEV_PUBLIC_KEY // fallback
  }

  // Build producer schedule: map each producer name to the signing key of the node that hosts it
  const prodSchedule: Array<{ producer_name: string; block_signing_key: string }> = []
  for (const name of producerNames.slice(0, 21)) {
    // Find the node that produces this name
    const hostNode = nodeStates.find(ns => ns.cmd.some(a => a === name && ns.cmd.includes("--producer-name")))
      ?? nodeStates.find(ns => {
        ...
        for (let i = 0; i < ns.cmd.length; i++) {
          if (ns.cmd[i] === "--producer-name" && ns.cmd[i + 1] === name) return true
        }
        return false
      })
    const sigKey = hostNode ? extractNodeK1PubKey(hostNode) : DEV_PUBLIC_KEY
    prodSchedule.push({ producer_name: name, block_signing_key: sigKey })
  }

  await clio.pushAction("sysio", "setprodkeys", JSON.stringify({ schedule: prodSchedule }), "sysio@active")
  ...
  while (Date.now() < handoffDeadline) {
    try {
      ...
      if (info.head_block_producer && info.head_block_producer !== "sysio") {
        log.info(`[Phase 8] Producer handoff: ${info.head_block_producer as string}`)
  ...
  await retry(
    () => clio.setContractAndWait("sysio.token", resolveContractDir("sysio.token"), "sysio.token.wasm", "sysio.token.abi"),
  ...
  await clio.pushActionAndWait("sysio.token", "create", JSON.stringify({ issuer: "sysio", maximum_supply: "1000000000.0000 SYS" }), "sysio.token@active")
  await clio.pushActionAndWait("sysio.token", "issue", JSON.stringify({ to: "sysio", quantity: "1000000000.0000 SYS", memo: "initial issue" }), "sysio@active")
  for (const name of producerNames) {
    await clio.pushAction("sysio.token", "transfer", JSON.stringify({ from: "sysio", to: name, quantity: "1000000.0000 SYS", memo: "init" }), "sysio@active")
  }
  ...
  await retry(
    () => clio.setContractAndWait("sysio.roa", resolveContractDir("sysio.roa"), "sysio.roa.wasm", "sysio.roa.abi"),
    { label: "deploy sysio.roa", maxAttempts: 3, delayMs: 2000 }
  )
  await clio.setPriv("sysio.roa")
  await clio.pushAction("sysio.roa", "activateroa", JSON.stringify({ total_sys: "75496.0000 SYS", bytes_per_unit: "104" }), "sysio.roa@active")
  log.info("[Phase 11] sysio.roa deployed")

  // ── Phase 12: Deploy sysio.authex ──
  log.info("[Phase 12] Deploying sysio.authex...")
  await retry(
    () => clio.setContractAndWait("sysio.authex", resolveContractDir("sysio.authex"), "sysio.authex.wasm", "sysio.authex.abi"),
    { label: "deploy sysio.authex", maxAttempts: 3, delayMs: 2000 }
  )
  await clio.setPriv("sysio.authex")
  await clio.pushAction("sysio", "updateauth", JSON.stringify({
    account: "sysio.authex", permission: "owner", parent: "",
    auth: {
      threshold: 1,
      keys: [{ key: DEV_PUBLIC_KEY, weight: 1 }],
  ...
  await clio.pushAction("sysio", "init", JSON.stringify({ version: 0, core: "4,SYS" }), "sysio@active")
  ...
  for (const [contractName, relPath] of Object.entries(OPP_CONTRACT_PATHS)) {
    ...
    await retry(
      () => clio.setContractAndWait(contractName, contractDir, `${contractName}.wasm`, `${contractName}.abi`),
  ...
  const opsPerEpoch = batchOpGroups > 0 ? Math.ceil(batchOpMin / batchOpGroups) : 1
  const adjustedMin = opsPerEpoch * batchOpGroups
  await clio.pushAction("sysio.epoch", "setconfig", JSON.stringify({
  ...
  await clio.pushAction("sysio.epoch", "regoutpost", JSON.stringify({ chain_kind: 2, chain_id: 31337 }), "sysio.epoch@active")
  await clio.pushAction("sysio.epoch", "regoutpost", JSON.stringify({ chain_kind: 3, chain_id: 0 }), "sysio.epoch@active")
  log.info("[Phase 16] Outposts registered")

  // ── Phase 17: Configure sysio.uwrit ──
  log.info("[Phase 17] Configuring sysio.uwrit...")
  await clio.pushAction("sysio.uwrit", "setconfig", JSON.stringify({
    fee_bps: 10, confirm_lock_sec: 86400, uw_fee_share_pct: 50, other_uw_share_pct: 25, batch_op_share_pct: 25,
  }), "sysio.uwrit@active")
  ...
  for (const bo of batchOpStates) {
    ...
    try { await clio.createAccount("sysio", account, DEV_PUBLIC_KEY, DEV_PUBLIC_KEY) } catch {}
    await clio.pushAction("sysio.epoch", "regoperator", JSON.stringify({ account, type: 2 }), "sysio.epoch@active")
  }
  ...
  for (const uw of underwriterStates) {
    ...
    try { await clio.createAccount("sysio", account, DEV_PUBLIC_KEY, DEV_PUBLIC_KEY) } catch {}
    await clio.pushAction("sysio.epoch", "regoperator", JSON.stringify({ account, type: 3 }), "sysio.epoch@active")
  }
