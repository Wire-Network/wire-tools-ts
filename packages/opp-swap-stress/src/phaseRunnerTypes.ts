...
/**
 * OPP endpoint directions whose saturation defines swap-stress ramp success.
 *
 * The two Ethereum legs of the depot↔outpost round trip. This is the single
 * swap-scenario source of truth for the required endpoint set (previously
 * duplicated as two hardcoded helpers in `rampController` and
 * `rampCampaignSaturation`); the generic ramp and campaign-saturation
 * machinery consume it. Changing this set changes which directions must
 * saturate before the ramp advances.
 */
export const SwapStressRequiredEndpoints: readonly RunEvidenceEndpoint[] = [
