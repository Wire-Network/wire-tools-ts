...
async function readT5State(ctx: ClusterBuildContext): Promise<SysioContracts.SysioSystemT5StateType> {
  const { rows } = await ctx.wire.getSysioContract(SysioContractName.system).tables.t5state.query()
  Assert.ok(rows.length >= 1, "sysio::t5state singleton row missing")
  return rows[0]
}

/** The `sysio::emitcfg` singleton row (a read; asserts the row exists). */
async function readEmissionConfig(ctx: ClusterBuildContext): Promise<SysioContracts.SysioSystemEmissionConfigType> {
  const { rows } = await ctx.wire.getSysioContract(SysioContractName.system).tables.emitcfg.query()
...
async function readCapConfig(ctx: ClusterBuildContext): Promise<SysioContracts.SysioDclaimCapConfigType> {
  const { rows } = await ctx.wire.getSysioContract(SysioContractName.dclaim).tables.capcfg.query()
...
export class EmissionsSoakScenario extends FlowScenario {
  ...
  plan(cluster: ClusterBuild): void {
    ...
    const identities = buildControlledStakerIdentities(
      ...
      ethereumConversion = toSeedConversionSummary(convertImportSeed(ethereumDump, { chain: Constants.EthereumChain })),
      solanaConversion = toSeedConversionSummary(convertImportSeed(solanaDump, { chain: Constants.SolanaChain })),
    ...
    ClusterBuildPhase.create(
      ...
      verifyStep(
        ...
        async ctx => {
          ...
          Assert.strictEqual(config.governance_bps, Constants.ExpectedGovernanceBps, "governance_bps drifted")
      ...
      verifyStep(
        ...
        async ctx => {
          ...
          Assert.strictEqual(Number(t5.capital_shortfall_total), 0, "capital_shortfall_total must start at 0")
        }
      )
    )
      ...
      verifyStep(
        ...
        async ctx => {
          ...
          roster.forEach(identity => {
            const credit = credits.find(candidate => candidate.native_address === identity.addressHex)
            Assert.ok(credit != null, `controlled staker ${identity.wireAccount} missing from the importseed credits`)
            Assert.strictEqual(
              credit.wire_atomic,
              expectedAtomic,
      ...
      verifyStep(
        ...
        async ctx => {
          ...
          await pollUntil(
            ...
            async () => {
              const { rows } = await ctx.wire.getSysioContract(SysioContractName.dclaim).tables.pclaims.query()
              return rows.filter(row => linkedAccounts.has(row.wire_account)).length === roster.length
            },
    ...
    ClusterBuildPhase.create(
      ...
      verifyStep(
        ...
        async (ctx, signal) => {
          ...
          Assert.ok(lastDistributed <= headroom, `total_distributed ${lastDistributed} exceeds t5 headroom ${headroom}`)
          Assert.ok(lastDistributed > startDistributed, "no emissions accrued across the soak window")
        },
        soakOptions
      )
    )
    ...
    ClusterBuildPhase.create(cluster, "Claim", "Each controlled staker claims and receives its exact seeded WIRE").push(
      EmissionsSoakScenarioSteps.planUnlockWallet(
        Actor.User,
        "unlock-wallet-claim",
      ...
      verifyStep(Actor.User, "snapshot-preclaim-balances", "record each staker's WIRE balance", async ctx => {
        const roster = ctx.outputs.assert(ClaimantIdentitiesKey),
          entries = await Promise.all(
            roster.map(
              async identity => [identity.wireAccount, await ctx.wire.getWireBalance(identity.wireAccount)] as const
            )
          )
