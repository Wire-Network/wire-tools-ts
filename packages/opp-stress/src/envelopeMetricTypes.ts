...
/**
 * Source-agnostic envelope record consumed by saturation projection.
 *
 * The narrow subset of a strict `ValidEnvelopePair` the metrics path actually
 * reads, so a non-filesystem source (on-chain RPC, remote debugging-server) can
 ...
 *
 * The source-agnostic input to `projectSnapshotSaturationMetrics`. `kind`
 * mirrors the strict reader's collected/failed outcome; `candidateCount` and
 * `issues` drive telemetry health exactly as the filesystem reader's candidate
...
export type EnvelopeMetricSnapshot = {
  ...
  /** Structured integrity issues; an on-chain source yields none. */
