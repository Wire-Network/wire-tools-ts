...
export const selectCurrentEpochIndex = (state: RootState): number => state[SliceName.OPP].epochIndex
...
export const selectLatestEpoch = (state: RootState): DebugOPPEpochRecord | undefined => {
 ...
 */
export const selectAllEpochsDescending = (state: RootState): DebugOPPEpochRecord[] => {
  const opp = state[SliceName.OPP]
